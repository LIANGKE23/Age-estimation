...
############################################### KE LIANG ###############################################################
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        # train
        train_loss, train_acc,train_mae = train(train_loader, model, criterion, optimizer, epoch, device)
        if args.tensorboard is not None:
        ...
        if train_mae < best_train_mae:
            print(f"=> [epoch {epoch:03d}] best val mae was improved from {best_train_mae:.3f} to {train_mae:.3f}")
            ...
            torch.save(
                ...
                str(checkpoint_dir.joinpath("epoch{:03d}_{:.5f}_{:.4f}.pth".format(epoch, train_loss, train_mae)))
            )
            best_train_mae = train_mae
        else:
            print(f"=> [epoch {epoch:03d}] best val mae was not improved from {best_train_mae:.3f} ({train_mae:.3f})")
